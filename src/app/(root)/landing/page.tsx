...
import { getAllEscrows } from "../../solana/solana";
...
const Landing = () => {
  //hooks
  const { data: escrows, error: escrowsError, isLoading } = getAllEscrows();
  ...
  return (
    <main className="flex flex-col scroll-smooth gap-y-10 min-h-screen items-center p-6 bg-transparent pt-[74px] relative overflow-y-auto">
      <div className="flex w-full h-full flex-wrap gap-x-4 gap-y-4 max-w-6xl mt-10">
        {escrows ? (
          <motion.div initial="hidden" animate="visible" variants={containerVariants} custom={1} className="flex flex-col gap-y-4 w-full">
