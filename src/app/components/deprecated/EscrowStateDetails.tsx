...
import { EscrowStateDetailsProps } from "@/app/interfaces";
...
const EscrowStateDetails: React.FC<EscrowStateDetailsProps> = ({
  ...
  useEffect(() => {
    ...
    fetchDecimals();
  }, [escrowState.mintA, escrowState.mintB, escrowState.tokenBalance]);
    ...
    <div className="flex flex-col gap-y-4 mt-4 p-4 border text-toekn-white border-toekn-white bg-transparent">
      <p>
        ...
        <span className="text-red-400">
          {escrowState.receive.words[0] / Math.pow(10, mintBDecimals)}{" "}
          {escrowState.mintBName || escrowState.mintB.toBase58()}
        </span>{" "}
        tokens to receive{" "}
        <span className="text-green-500">
          {escrowState.tokenBalance}{" "}
          {escrowState.mintAName || escrowState.mintA.toBase58()}
        </span>{" "}
        tokens?
      </p>
      <button
